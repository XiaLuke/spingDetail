...
public class DepartmentServiceTest {
    ...
    @Autowired
    private TransactionInvocationHandler departmentTransactionInvocationHandler;

    @Test
    public void test() {
        // 生成类，并返回对象，存到 proxy 变量上
        // 1、这里真是生成类创建这个类对象吗？
        IDepartmentService proxy = (IDepartmentService)Proxy.newProxyInstance(
                this.getClass().getClassLoader(), // 类加载器
                departmentTransactionInvocationHandler.getTarget()
                        .getClass().getInterfaces(),// 生成类要实现的接口
                departmentTransactionInvocationHandler // 生成类对象能做什么事情（调用事务管理器的方法，调用真实对象的方法，给业务类的方法增加模拟的事务）
        );
